...
export function deriveResultsFromMatches(matches: ScoredMatch[]): TournamentResults {
  ...
  // ── GROUP STAGE ──
  // Set group positions. Top 2 in each group + 8 best 3rd-place teams advance to R32.

  // Collect all 3rd-place teams to determine which 8 advance
  const allThirdPlaced: { team: string; pts: number; gd: number; gf: number }[] = [];
  ...
  for (const [, groupStandings] of standings) {
    for (let i = 0; i < groupStandings.length; i++) {
      ...
      if (i < 2) {
        ...
        // 3rd place — collect for ranking
        allThirdPlaced.push({ team: s.team, pts: s.points, gd: s.gd, gf: s.gf });
      }
      // 4th place — leave for now, will be marked eliminated below
    }
  }

  // Rank 3rd-place teams: best 8 advance, worst 4 are eliminated
  ...
  for (let i = 0; i < allThirdPlaced.length; i++) {
    if (i < 8) {
      results[allThirdPlaced[i].team].reachedKnockout = true;
    } else {
      results[allThirdPlaced[i].team].eliminated = true;
    }
  }

  // Mark all 4th-place teams as eliminated
  for (const [, groupStandings] of standings) {
    for (let i = 0; i < groupStandings.length; i++) {
      const s = groupStandings[i];
      if (i === 3 && s.played >= 1) {
