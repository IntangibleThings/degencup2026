...
function parseESPNFormat(md: string): ScrapedMatch[] {
  ...
  // Supports all dash types and 3-letter team codes
  const DASH = '[-\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE58\uFE63\uFF0D]';
  const TEAM = "[A-Za-z][A-Za-z\\s'\\.\\-]*[A-Za-z]|[A-Za-z]{2,}";
  const regex = new RegExp(`Group\\s+[A-L]:\\s*(${TEAM})\\s+(\\d+)\\s*${DASH}\\s*(\\d+)\\s+(${TEAM})(?:\\s*\\(|\\s*$|\\n)`, 'gi');
...
function parseBoldFormat(md: string): ScrapedMatch[] {
  ...
  const DASH = '[-\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE58\uFE63\uFF0D]';
  const TEAM = "[A-Za-z][A-Za-z\\s'\\.\\-]*[A-Za-z]|[A-Za-z]{2,}";
  const regex = new RegExp(`\\*\\*(${TEAM})\\*\\*\\s+\\*\\*(\\d+)\\*\\*\\s*${DASH}\\s+\\*\\*(\\d+)\\*\\*\\s+\\*\\*(${TEAM})\\*\\*`, 'gi');
...
function parsePlainFormat(md: string): ScrapedMatch[] {
  ...
  const DASH = '[-\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE58\uFE63\uFF0D]';
  const TEAM = "[A-Za-z][A-Za-z\\s'\\.\\-]*[A-Za-z]|[A-Za-z]{2,}";
  const regex = new RegExp(`^(${TEAM})\\s+(\\d+)\\s*${DASH}\\s*(\\d+)\\s+(${TEAM})(?:\\s+(?:FT|AET|PEN|FINISHED|Full Time))?\\s*$`, 'gim');
...
function parseWikiTableFormat(md: string): ScrapedMatch[] {
  ...
  // Wikipedia: "|France|3–1|Senegal|" with all dash types
  const DASH = '[-\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE58\uFE63\uFF0D]';
  const TEAM = "[A-Za-z][A-Za-z\\s'\\.\\-]*[A-Za-z]|[A-Za-z]{2,}";
  const regex = new RegExp(`\\|\\s*(${TEAM})\\s*\\|\\s*(\\d+)\\s*${DASH}\\s*(\\d+)\\s*\\|\\s*(${TEAM})\\s*\\|`, 'gi');
