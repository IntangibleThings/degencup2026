        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_ITEMS.map(item => (
            <Link key={item.path} to={item.path}
              ...
              style={{
                color: location.pathname === item.path ? '#FFD700' : '#8899AA',
                borderBottom: location.pathname === item.path ? '4px solid #FFD700' : '4px solid transparent',
              }}>
              {item.label}
            </Link>
          ))}
        ...
        <div className="fixed inset-0 z-40 flex flex-col items-center justify-center gap-6" style={{ backgroundColor: '#16213E' }}>
          {NAV_ITEMS.map(item => (
            <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)}
              className="font-pixel text-lg" style={{ color: location.pathname === item.path ? '#FFD700' : '#E8E8E8' }}>
              {item.label}
            </Link>
          ))}
        </div>
