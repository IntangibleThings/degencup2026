...
import { parsePastedScoresToMatrix, getMatrix, getMatrixStats, isMatchScoredLegacy as isMatchScored, type MatrixMatch } from '@/data/matchMatrix';
import { generateBracketSuggestions, seedKnockoutFromGroupResults, parseBracketSeed, autoAdvanceKnockoutWinners, R32_IDS, R16_IDS, QF_IDS, SF_IDS, THIRD_ID, FINAL_ID } from '@/data/bracketAdvancement';
...
export default function AdminPage() {
  const { state, dispatch, lockDraft, unlockDraft, warnManager, setManagerActive, resetManagerCode, forceSync, addManager, loadFromCloud, firebaseEnabled, removeManager, saveSettings, saveResults, resolveWager, loadSeedData, deleteWager, toggleWagerComments, updateMatchScore } = useGame();
    ...
    if (matches.length > 0) {
      ...
      for (const m of matches) {
        if (m.homeGoals !== null && m.awayGoals !== null) {
          ...
          const mat = getMatrix();
          const match = mat.find(
            (x: MatrixMatch) => (x.homeTeam === m.homeTeam && x.awayTeam === m.awayTeam) ||
  ...
  const handleApplyDerivedResults = async () => {
    ...
    localStorage.setItem('wc2026_derived_at', new Date().toISOString());
    const stats = getMatrixStats(getMatrix());
  ...
  const handleApplyPastedScores = () => {
    ...
    // Parse pasted text to find which scores match our fixture list
    const result = parsePastedScoresToMatrix(pasteText, getMatrix());

    // CRITICAL: Actually apply each matched score to the matrix through GameContext
    // This ensures matrixRef is updated and results are re-derived
    let applied = 0;
    for (const match of result.matched) {
      const success = updateMatchScore(match.matchId, match.homeGoals, match.awayGoals);
      if (success) applied++;
    }
    ...
    const statusMsg = result.overwritten > 0
      ? `APPLIED ${applied} SCORES (${result.overwritten} UPDATED) | ${stats.scoredMatches}/${stats.totalMatches} TOTAL FILLED`
      : `APPLIED ${applied} NEW SCORES | ${stats.scoredMatches}/${stats.totalMatches} TOTAL FILLED`;
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {activeTab === 'sync' && (
          <div className="space-y-4">
            {/* EMERGENCY: Reset all local data */}
            <div className="retro-card p-4" style={{ borderColor: '#E60012' }}>
              <h3 className="font-pixel text-[10px] mb-2 flex items-center gap-2" style={{ color: '#E60012' }}>
                <RefreshCw className="w-3 h-3" /> EMERGENCY RESET
              </h3>
              <p className="font-pixel text-[7px] mb-2" style={{ color: '#8899AA' }}>
                If scores flash then revert, stale data is cached. Click RESET to clear all local data and reload fresh. Your managers and settings are safely stored in Firebase.
              </p>
              <button
                onClick={() => {
                  if (confirm('CLEAR ALL LOCAL DATA AND RELOAD?\n\nThis fixes the score flash bug by removing stale cached results.\nYour managers/settings in Firebase are safe.')) {
                    localStorage.clear();
...
France 3-1 Senegal`}
            ...
            {/* R16 Bracket Editor — Manual override for Round of 16 matchups */}
            <R16BracketEditor />

            {/* QF Bracket Editor — Manual override for Quarter Final matchups */}
            <QFBracketEditor />

            {/* SF Bracket Editor — Manual override for Semi Final matchups */}
            <SFBracketEditor />

            {/* 3rd Place Bracket Editor */}
            <ThirdPlaceBracketEditor />

            {/* Final Bracket Editor */}
            <FinalBracketEditor />
...
function MatrixStatsBar() {
  ...
  useEffect(() => {
    const interval = setInterval(() => setStats(getMatrixStats(getMatrix())), 1000);
...
function KnockoutBracketManager() {
  const { assignKnockoutTeam } = useGame();
  ...
  const handleApplyBracketPaste = () => {
    ...
    for (const s of parsedSeeds) {
      assignKnockoutTeam(s.matchId, 'home', s.home, s.homeName);
      assignKnockoutTeam(s.matchId, 'away', s.away, s.awayName);
    }
    setBracketPasteText('');
    setParsedSeeds(null);
    refresh();
  ...
  const mat = getMatrix();
  const scoredKo = mat.filter(m => !m.round.startsWith('GROUP_') && m.homeGoals !== null);
...
function R32BracketEditor() {
  ...
  const handleClear = () => {
    ...
    for (const id of R32_IDS) {
      assignKnockoutTeam(id, 'home', 'None', 'None');
      assignKnockoutTeam(id, 'away', 'None', 'None');
    }
    refresh();
...
// ============================================================================
// R16 BRACKET EDITOR — Direct manual control of all 8 Round of 16 matchups
// ============================================================================

function R16BracketEditor() {
  const { assignKnockoutTeam } = useGame();
  const [isOpen, setIsOpen] = useState(false);
  const [matchSlots, setMatchSlots] = useState<{ matchId: number; homeCode: string; homeName: string; awayCode: string; awayName: string }[]>([]);
  const [bulkText, setBulkText] = useState('');
    ...
    lines.forEach((line, idx) => {
      ...
      for (const pattern of patterns) {
        ...
        if (match) {
          ...
          const homeCode = mapTeamName(home) || home.toUpperCase();
          const awayCode = mapTeamName(away) || away.toUpperCase();
          newSlots[idx] = { ...newSlots[idx], homeCode, homeName: homeCode, awayCode, awayName: awayCode };
          break;
        }
      }
    });
  ...
  const handleClear = () => {
    if (!confirm('CLEAR ALL R16 SLOT ASSIGNMENTS?')) return;
    for (const id of R16_IDS) {
      assignKnockoutTeam(id, 'home', 'None', 'None');
      assignKnockoutTeam(id, 'away', 'None', 'None');
    }
    refresh();
    setSavedMsg('CLEARED ALL R16 SLOTS');
    setTimeout(() => setSavedMsg(''), 3000);
  };
  ...
  return (
    <div className="retro-card p-4" style={{ borderColor: '#00c8ff' }}>
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-pixel text-[10px] flex items-center gap-2" style={{ color: '#00c8ff' }}>
          <Trophy className="w-3 h-3" /> ROUND OF 16 EDITOR
        </h3>
        <div className="flex gap-2">
          {filledCount > 0 && (
            <span className="font-pixel text-[7px] px-2 py-1" style={{ backgroundColor: 'rgba(0,200,255,0.2)', color: '#00c8ff' }}>
              {filledCount}/8 FILLED
            </span>
          )}
          <button onClick={toggleOpen}
            className="pixel-btn small" style={{ borderColor: '#00c8ff', color: '#00c8ff' }}>
            {isOpen ? 'HIDE' : 'EDIT'}
          </button>
        </div>
      </div>
      ...
      {!isOpen && (
        <p className="font-pixel text-[7px]" style={{ color: '#8899AA' }}>
          Manually set all 8 Round of 16 matchups. Use after auto-advance or to fix wrong pairings.
        </p>
      )}
      ...
      {isOpen && (
        <div className="space-y-3">
          <div className="p-2" style={{ backgroundColor: 'rgba(45,49,146,0.1)', border: '1px solid #0F3460' }}>
            <p className="font-pixel text-[7px] mb-1" style={{ color: '#00c8ff' }}>BULK PASTE (8 lines)</p>
            <p className="font-pixel text-[6px] mb-1" style={{ color: '#8899AA' }}>
              Format: &quot;MEX vs NED&quot; (line 1 = R16-1, line 2 = R16-2, etc.)
            </p>
            <textarea value={bulkText} onChange={e => setBulkText(e.target.value)}
              placeholder={`MEX vs NED\nARG vs DEN\nBRA vs CRO`}
              className="pixel-input w-full text-[10px] resize-none" rows={3} />
          ...
          <button onClick={handleApply}
            className="pixel-btn green w-full flex items-center justify-center gap-2">
            <Check className="w-3 h-3" /> SAVE ALL R16 MATCHUPS TO MATRIX
          </button>
        </div>
      )}
    </div>
...
// ============================================================================
// GENERIC KNOCKOUT EDITOR — Reused for QF, SF, 3rd Place, Final
// ============================================================================
...
function KnockoutEditor({ config }: { config: EditorConfig }) {
  const { assignKnockoutTeam } = useGame();
  const { title, ids, borderColor, labelColor, labelShort } = config;
  const [isOpen, setIsOpen] = useState(false);
  const [matchSlots, setMatchSlots] = useState<{ matchId: number; homeCode: string; homeName: string; awayCode: string; awayName: string }[]>([]);
  const [bulkText, setBulkText] = useState('');
  const [savedMsg, setSavedMsg] = useState('');
  ...
  const toggleOpen = () => { if (!isOpen) refresh(); setIsOpen(!isOpen); };
  ...
  const handleParseBulk = () => {
    ...
    lines.forEach((line, idx) => {
      ...
      const match = line.match(/^(.+?)\s+(?:\d+[-\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE58\uFE63\uFF0D]\d+\s+)?vs\s+(.+)$/i)
        || line.match(/^(.+?)\s+\d+[-\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE58\uFE63\uFF0D]\d+\s+(.+)$/);
      if (match) {
        const home = match[1].trim();
        const away = match[2].trim();
  ...
  const handleClear = () => {
    if (!confirm(`CLEAR ALL ${labelShort} SLOT ASSIGNMENTS?`)) return;
    for (const id of ids) {
      assignKnockoutTeam(id, 'home', 'None', 'None');
      assignKnockoutTeam(id, 'away', 'None', 'None');
    }
    refresh();
    setSavedMsg(`CLEARED ALL ${labelShort} SLOTS`);
    setTimeout(() => setSavedMsg(''), 3000);
  };
  ...
  return (
    <div className="retro-card p-4" style={{ borderColor }}>
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-pixel text-[10px] flex items-center gap-2" style={{ color: labelColor }}>
          <Trophy className="w-3 h-3" /> {title}
        </h3>
        <div className="flex gap-2">
          {filledCount > 0 && (
            <span className="font-pixel text-[7px] px-2 py-1" style={{ backgroundColor: labelColor + '22', color: labelColor }}>
              {filledCount}/{ids.length} FILLED
            </span>
          )}
          <button onClick={toggleOpen} className="pixel-btn small" style={{ borderColor: labelColor, color: labelColor }}>
            {isOpen ? 'HIDE' : 'EDIT'}
          </button>
        </div>
      </div>
      ...
      {!isOpen && (
        <p className="font-pixel text-[7px]" style={{ color: '#8899AA' }}>
          Manually set {ids.length === 1 ? 'the matchup' : `all ${ids.length} matchups`}. Use after auto-advance or to fix wrong pairings.
        </p>
      )}
      ...
      {isOpen && (
        <div className="space-y-3">
          <div className="p-2" style={{ backgroundColor: 'rgba(45,49,146,0.1)', border: '1px solid #0F3460' }}>
            <p className="font-pixel text-[7px] mb-1" style={{ color: '#00c8ff' }}>BULK PASTE ({ids.length} {ids.length === 1 ? 'line' : 'lines'})</p>
            <p className="font-pixel text-[6px] mb-1" style={{ color: '#8899AA' }}>Format: &quot;MEX vs NED&quot;</p>
            <textarea value={bulkText} onChange={e => setBulkText(e.target.value)}
              placeholder={ids.length === 1 ? 'MEX vs NED' : 'MEX vs NED\nARG vs BRA'}
              className="pixel-input w-full text-[10px] resize-none" rows={Math.min(ids.length, 4)} />
            <div className="flex gap-2 mt-1">
              <button onClick={handleParseBulk} disabled={!bulkText.trim()}
                className="pixel-btn small" style={{ borderColor: '#00c8ff', color: '#00c8ff' }}>PARSE & FILL</button>
              <button onClick={handleClear} className="pixel-btn red small text-[6px]">CLEAR ALL</button>
            </div>
          </div>
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto">
            ...
            {matchSlots.map((slot, i) => (
              <div key={slot.matchId} className="grid grid-cols-12 gap-1 px-1 py-1 items-center"
                ...
                <input type="text" value={slot.homeCode}
                  onChange={e => updateSlot(slot.matchId, 'homeCode', e.target.value.toUpperCase())}
                  placeholder="MEX" maxLength={3} className="col-span-2 pixel-input text-[8px] py-1 px-1" />
                <input type="text" value={slot.homeName}
                  onChange={e => updateSlot(slot.matchId, 'homeName', e.target.value)}
                  placeholder="Mexico" className="col-span-3 pixel-input text-[8px] py-1 px-1" />
                <input type="text" value={slot.awayCode}
                  onChange={e => updateSlot(slot.matchId, 'awayCode', e.target.value.toUpperCase())}
                  placeholder="NED" maxLength={3} className="col-span-2 pixel-input text-[8px] py-1 px-1" />
                <input type="text" value={slot.awayName}
                  onChange={e => updateSlot(slot.matchId, 'awayName', e.target.value)}
                  placeholder="Netherlands" className="col-span-3 pixel-input text-[8px] py-1 px-1" />
                <div className="col-span-1 font-pixel text-[5px]" style={{ color: '#556677' }}>{slot.matchId}</div>
              </div>
            ))}
          </div>
          ...
          <button onClick={handleApply}
            className="pixel-btn green w-full flex items-center justify-center gap-2">
            <Check className="w-3 h-3" /> SAVE ALL {labelShort} MATCHUPS TO MATRIX
          </button>
        </div>
      )}
    </div>
...
function QFBracketEditor() {
  return <KnockoutEditor config={{ title: 'QUARTER FINALS EDITOR', ids: QF_IDS, borderColor: '#FF6B35', labelColor: '#FF6B35', labelShort: 'QF' }} />;
}

function SFBracketEditor() {
  return <KnockoutEditor config={{ title: 'SEMI FINALS EDITOR', ids: SF_IDS, borderColor: '#9B59B6', labelColor: '#9B59B6', labelShort: 'SF' }} />;
}

function ThirdPlaceBracketEditor() {
  return <KnockoutEditor config={{ title: '3RD PLACE EDITOR', ids: [THIRD_ID], borderColor: '#E67E22', labelColor: '#E67E22', labelShort: '3RD' }} />;
}

function FinalBracketEditor() {
  return <KnockoutEditor config={{ title: 'FINAL EDITOR', ids: [FINAL_ID], borderColor: '#FFD700', labelColor: '#FFD700', labelShort: 'FINAL' }} />;
}
