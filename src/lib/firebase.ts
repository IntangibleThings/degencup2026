...
// ========================================================================
// COMMENT OPERATIONS (Degen Den chatter)
// ========================================================================

const COMMENTS_COL = `${GAME_DOC}/comments`;

export async function saveComment(comment: Record<string, unknown>): Promise<void> {
  if (!db) throw new Error('Firebase not configured');
  await setDoc(doc(db, COMMENTS_COL, comment.id as string), comment);
  console.log('[FIREBASE] Saved comment:', comment.id);
}

export async function loadAllComments(): Promise<Record<string, unknown>[]> {
  if (!db) throw new Error('Firebase not configured');
  const snap = await getDocs(collection(db, COMMENTS_COL));
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

export function subscribeToComments(
  callback: (comments: Record<string, unknown>[]) => void
): () => void {
  if (!db) return () => {};
  const unsub = onSnapshot(collection(db, COMMENTS_COL), (snap) => {
    const comments = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    console.log('[FIREBASE] Comments update, count:', comments.length);
    callback(comments);
  }, (err) => {
    console.error('[FIREBASE] Comments sub error:', err);
  });
  return unsub;
}
